...
1. **Argument of perigee / RAAN at injection.** Not published for any GOES flight. The screening problem assumes apogee occurs at an equatorial node (so the apogee burn combines plane change with perigee raise); the tudatpy stage must be seeded with an assumed arg-of-perigee, and results depend on it. **Assumption, unverified.** The absolute orientation of that assumed node *within inertial space* is no longer arbitrary, though: `viz/geo_raising.py::_launch_site_rotation` orients the whole exported trajectory so it launches from Cape Canaveral SLC-41's real longitude at a real liftoff epoch (`launch_epoch_mjd2000`), and every inclination in this module is measured against Earth's real equator (via `orbitopt.bodies.earth_body_fixed_to_eclipj2000`), not ECLIPJ2000's ecliptic-referenced z-axis (confirmed by rendering to previously land the trail over open ocean near the equator, ~23.4 degrees of inclination error baked in). The *shape* of the assumed node placement (equatorial, apsis-preserving) is unchanged and still the open assumption; only *where in the real world* that assumed geometry sits is now grounded.
